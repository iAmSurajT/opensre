...
Requires the [GitHub Copilot CLI](https://docs.github.com/copilot/how-tos/use-copilot-agents/use-copilot-cli) (`npm i -g @github/copilot`). Login uses the interactive `/login` slash command and stores credentials under `$COPILOT_HOME` (default `~/.copilot`); OpenSRE detects that directory as the primary auth signal. `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, and `GITHUB_TOKEN` are forwarded as auth fallbacks when present. If `COPILOT_MODEL` is unset, OpenSRE omits the `--model` flag and the CLI uses its configured default. Invocations run as `copilot -p PROMPT --no-color --no-banner --no-ask-user --silent` so they never block on user input.
